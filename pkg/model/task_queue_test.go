...
func TestQueue(t *testing.T) {
	...
	qLen := queue.Len()
	if qLen != 1 {
		t.Errorf("queue.Len() should be 1, now it's %d", qLen)
	}

	n := queue.GetUnfinished()
	if n != 2 {
		t.Errorf("queue.GetUnfinished() should be 2, now it's %d", n)
	}

	queue.FinishOneTask()
	n = queue.GetUnfinished()
	if n != 1 {
		t.Errorf("queue.GetUnfinished() should be 1, now it's %d", n)
	}
